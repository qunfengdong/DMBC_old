...
loocv <- function(data=data, type_col=2, col_start=3,Cutoff_mean=0.0005,Cutoff_ratio=0.1,totalReadsCutoff=500){
  res <- list()
  for (i in 1:nrow(data)){
    testSet <- data[i,]
    training <- data[-i,]
    FS_out <- FS(training=training,type_col=type_col,col_start=col_start,Cutoff_mean=Cutoff_mean,Cutoff_ratio=Cutoff_ratio,totalReadsCutoff=totalReadsCutoff)
